...
protected:
    ...
    vertex* out_vertex[graph_size];   //能够出去的鳄鱼
    vertex* in_vertex[graph_size];    //起始点的鳄鱼
...
template<class Weight,int graph_size>
void digraph<Weight, graph_size>::set_stepsize(double num){
    step_size=num;
}
...
bool digraph<Weight, graph_size>::is_neighbor(vertex *p_v1, vertex *p_v2){          //给定两个结点，判断是否相邻
    if(pow(pow(p_v1->data->x-p_v2->data->x,2)+pow(p_v1->data->y-p_v2->data->y,2),0.5)<=step_size){
...
void digraph<Weight, graph_size>::update_related(){
    ...
    p_ver=root_vertex;
    for(int i=0;i<graph_size;i++){
        if(pow(pow(p_ver->data->x,2)+pow(p_ver->data->y,2),0.5)<=7.5+step_size){
            in_vertex[in_count++]=p_ver;
        }
        if(p_ver->data->x+step_size>=50||p_ver->data->x-step_size<=-50||p_ver->data->y+step_size>=50||p_ver->data->y-step_size<=-50){
            out_vertex[out_count++]=p_ver;
        }
        p_ver=p_ver->next_vertex;
    }
...
void digraph<Weight,graph_size>::run(){
    ...
    if(step_size>=42.5){
        cout<<"最短的步数为："<<1<<",直接跨过去"<<endl;
        return ;
    }
    int in_index,out_index;
    for(int i=0;in_vertex[i]!=NULL;i++){
        for(int j=0;out_vertex[j]!=NULL;j++){
            if(min>return_shorst_wight(in_vertex[i], out_vertex[j])&&return_shorst_wight(in_vertex[i], out_vertex[j])>=0){
                in_number=in_vertex[i]->mark_number;    //记录最短路径的结点的mark_number
                out_number=out_vertex[j]->mark_number;
                in_index=i;
                out_index=j;
                min=return_shorst_wight(in_vertex[i], out_vertex[j]);
            }
        }
    }
    if(min==9999)
        ...
        return_shorst_wight(in_vertex[in_index], out_vertex[out_index]);   //重新跑一次最短路径的起点和终点，对
                                                                           //path_vertex进行正确的初始化
        cout<<"最短的步数为:"<<min+2<<endl;
        cout<<"路径为：";
